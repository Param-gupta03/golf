...
export const api = {
  ...
  createCheckoutSession: async (userId, email, plan) => {
  ...
  verifyCheckoutSession: async (sessionId, userId, plan) => {
    const params = new URLSearchParams({
      sessionId,
      userId,
      plan,
    });
    const res = await fetch(`${BASE_URL}/payments/verify?${params.toString()}`);
    return res.json();
  },
